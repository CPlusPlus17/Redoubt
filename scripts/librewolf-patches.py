...
def android_brand_images():
    # LW-M4-07: replace Fenix's Firefox brand imagery with Redoubt's.
    #
    # The string layer was already clean -- LW-M4-12 rewrote the string table and
    # --check-strings passes with zero unexplained brand values. It has no view of
    # IMAGES, and neither did anything else, so the signed 2026-09-08 beta candidate
    # still carried the Firefox flame as its home-screen wordmark, the word
    # "Firefox" beside it, and eight user-selectable Firefox app icons behind
    # Settings. Measured out of that APK, not inferred from the sources.
    #
    # A copy rather than diff hunks, for the same reason as the search-config
    # dumps: these are 76 generated files, several of them binary, and a rebase
    # must not be able to reject on them. The bytes are checked in under
    # assets/android-brand/ and regenerated by scripts/gen-android-brand.py.
    #
    # Every destination is asserted to exist BEFORE anything is written (landmine
    # L4): a cp into a path upstream has renamed succeeds, changes nothing, and
    # would ship the Firefox art with every gate still green -- which is exactly
    # how this survived to a signed candidate.
    source_root = REPO_DIR / "assets" / "android-brand"
    if not source_root.is_dir():
        print("fatal error: {} is missing -- regenerate it with "
              "scripts/gen-android-brand.py <tree>".format(source_root))
        sys.stdout.flush()
        script_exit(1)

    replacements = sorted(p for p in source_root.rglob("*") if p.is_file())
    if not replacements:
        print("fatal error: {} contains no brand images".format(source_root))
        sys.stdout.flush()
        script_exit(1)
    ...
    if missing:
        print("fatal error: LW-M4-07 expects {} Firefox brand file(s) that are not in "
              "the tree at {}:".format(len(missing), os.getcwd()))
        for destination in missing[:10]:
            print("    {}".format(destination))
        if len(missing) > 10:
            print("    ... and {} more".format(len(missing) - 10))
        print("  Nothing was written. Upstream has renamed or removed them; re-run")
        print("  scripts/gen-android-brand.py against the new tree so the replacement")
        print("  set matches, then check the result in. See landmine L4 in")
        print("  docs/android/AGENTS.md.")
        sys.stdout.flush()
        script_exit(1)
    ...
    print("replaced {} Firefox brand image(s) with Redoubt's "
          "(assets/android-brand/)".format(len(replacements)))
    sys.stdout.flush()
