...
successful compiler evidence remain in `evidence/lw-m7-21/apk-bundle-resource-check-failure/`. Full
...
| F24 | Honest gates, build and upgrade | VM compiled the uBO/defaults candidate: Fenix gate 602 classes/5468 tests, 90 failures = 87 environmental + 3 known-real + 0 unexpected; 53 support-extension tests pass. Four development-signed release-type APKs built; x86_64 passes 8 baseline checks and a 20-key live pref audit. uBO immediate restart still fails | The cookie/graphics/translation increment now compiles into four development APKs; resource verification, full target tests and behavior checks remain pending. Curated audit does not cover all locks or UI. New shipping version and old-beta upgrade remain pending; preserved ambiguous old defaults need an explicit apply-defaults option |
