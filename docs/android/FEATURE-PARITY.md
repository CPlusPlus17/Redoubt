...
translations, Sync, cookie controls and Suggest admission. The first APK build then failed on Suggest data-class copy visibility; a
reviewed two-file Kotlin correction is queued for an APK retry. Full
Fenix/target suites remain pending. Later permission persistence, explicit
Suggest data import and global/extension controls require another candidate. These older APK results do not validate them.
