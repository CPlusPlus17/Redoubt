...
The later integrated source is rebuilding; these older APK results do not validate it.
...
| F12 | Cookie banner rejection | LW-M7-13 implements a pinned 558-rule Android local loader with no cookie RS client and unchanged allowlists; 23 actual-source tests pass. Source is integrated and native build is in progress | Exact APK assets and target tests; real reject callback or opt-out cookie with accept-only and allowed controls, exceptions, existing consent and normal/private lifecycle remain pending. Hidden banners are not rejection proof |
...
| F23 | Desktop patch/policy coverage | LW-M7-17 audits all 36 desktop patches/46 effect groups, 22 policy keys/46 exact leaves and 16 copied-pane controls. Mechanical coverage and negative controls pass; current graphics/translations source is reviewed | Close mapped functional gaps and run behavior checks. Input coverage is complete for these pinned inventories; counterpart implementation and runtime coverage are not |
