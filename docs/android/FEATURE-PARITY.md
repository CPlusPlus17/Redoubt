...
|---|---|---|---|
| F01 | uBlock Origin enabled by default | Pinned ordinary AMO-signed 1.74.0 install and live listener wait are compiled; full Fenix and 53 extension-support tests pass. Fresh first navigation blocks the matching script before automation and loads its allowed control | Beta 2 regression: every relaunch with uBO enabled deadlocked into "uBlock Origin setup failed" (Gecko delays the background until a window paints; the gate held every window). Source fix and `ubo-enabled-restart` smoke check 2026-09-24, not yet built or run (LW-M3-07). Immediate force-stop after completed disable exposes stale addonStartup cache and resumes real blocking despite disabled registry state. LW-M7-19 source fix and 36 source tests are in the current native build; UI disable/removal, private permissions, update/upgrade, tamper/failure and broader lifetimes remain open |
...
| F17 | Feedback, support links and messaging | **2026-09-24:** `no-webcompat-reporter.patch` (LW-M7-38) removes "Report broken site", whose Glean ping had no upload path; Fenix compilation pending. Much Mozilla messaging removed; actual reporter/menu/support routes still need review. `WebCompatFeature` is compatibility interventions, not a reporter | Remove/redirect actual feedback routes as desktop policies require, keep compatibility interventions working, verify visible menu targets |
