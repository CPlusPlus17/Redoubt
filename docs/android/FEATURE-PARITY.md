...
`evidence/lw-m7-21/second-current-target-failure/`; the full suite is being rerun
with the separate fifth test-only overlay. Later permission persistence, explicit
...
| F13 | Sync/accounts opt-in and disable | LW-M7-20 source implements off-by-default admission, explicit enable/disable and process restart, preserved account/engine/server data, and worker/callback gates. Root replay passes23 files and Fenix production code compiles;19 selected account/worker component tests pass after correcting a primitive Mockito matcher; the9 Fenix account tests await the full suite verdict | Compile and run target tests/full Fenix gate, then real login/Sync, PID replacement, private-session lifetime, failed persistence, disabled traffic and send/receive polling on the final APK |
