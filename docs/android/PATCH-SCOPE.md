...
Current classification: **24 common / 21 android / 36 desktop-only / 0 straddlers = 81
...
| `android.txt` | 21 | the three Android-side patches the M1 splits pulled in, plus `build-fixes` (LW-M2-02), `appservices-logins-addmany` (LW-M2-04), `no-nimbus` (LW-M4-03), `no-nimbus-toolkit` (LW-M4-13), `isolated-process` (LW-M5-02), `autoconfig-resource-fallback` (LW-M3-08/LW-M3-02), `no-onboarding` (LW-M4-10), `no-gms` (LW-M4-05), `branding` (LW-M4-07), `kotlin-version-alignment` (LW-M3-12) and the M4 dependency removals landing alongside it — one row each in the table below. |
...
| `patches/android/kotlin-version-alignment.patch` | LW-M3-12 | **Aligns the Kotlin version to 2.3.20.** `gradle/libs.versions.toml:8` pins the Kotlin Gradle plugin to 2.3.21, but the build runs on Gradle 9.5.1, whose embedded kotlin-dsl and embedded-kotlin plugins rely on Kotlin 2.3.20. The build log flags this mismatch itself: "Using the `kotlin-dsl` plugin together with a different Kotlin version ... is not recommended. Applying other version of the `org.jetbrains.kotlin.jvm` plugin ... can cause incompatibilities." The observed failure is a Kotlin-plugin-internal build-service registration type conflict on `:geckoview:compileDebugKotlin` (a DEBUG task run by `mach android export` for geckoview API extraction): `Cannot set the value of a property of type ...statistics.BuildFusService using a provider of type ...statistics.FlowActionBuildFusService`. **THE FAULT IS INTERMITTENT — attribution is unproven.** It hit x86_64 on 2026-08-22 then passed on rerun with no version change, and hit arm64-v8a + armeabi-v7a first passes on 2026-08-24 in clean fresh outdirs. This patch removes the real version mismatch (a defensible correctness fix) but does NOT claim the fault is fixed — evidence is reported as "mismatch removed", NOT "fault fixed". One line in `gradle/libs.versions.toml` [versions]. Order-free with `no-adjust` + `no-gms` (neither touches the `kotlin =` line at line 8) — the two pairs are recorded in `scripts/check-patch-order.py`'s `REVIEWED_ORDER_FREE` and the `docs/android/AGENTS.md` ordering table. |
