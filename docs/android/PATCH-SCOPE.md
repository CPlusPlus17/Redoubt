...
Current classification: **23 common / 46 android / 37 desktop-only / 0 straddlers = 106
...
| `patches/android/extension-permission-durability.patch` | LW-M7-31 | Android permission changes await atomic flushed JSON writes, preserve failed-write retries and read authoritative permission storage. GeckoView uninstall awaits its original permission cleanup task. Root ran 24 actual-source and 13 runner checks, plus all 32 UI harness checks; target packaging, native tests and immediate-restart behavior remain pending. |
...
| `patches/android/interrupted-session-cleanup.patch` | LW-M7-37 | Runs the delete-on-quit selection on every cold start before any engine session exists, awaiting each Gecko `clearData` callback, so a swiped-away or killed session is cleaned as LibreWolf desktop cleans on close. Holds sessions through a second `LibreWolfUboPreinstallMiddleware`; failures and the 30 s bound are logged and retried next start. Eleven JVM tests plus two mutation controls pass against signature stubs; Fenix compilation and device behaviour pending. Per-site retention not implemented. |
| `patches/android/no-webcompat-reporter.patch` | LW-M7-38 | Removes the "Report broken site" menu entry: its report is a Glean ping and Glean upload is removed, so submissions were silently discarded. LibreWolf's DisableFeedbackCommands removes the same command. Reporter code and navigation stay; WebCompatFeature interventions are unaffected. Fenix compilation pending. |
| `patches/android/drm-permission-default.patch` | LW-M7-39 | Fenix's DRM site permission defaults to Blocked (`PhoneFeature.getDefault`, the site-permission rules and `SettingsTest`'s fixture). With `media.eme.require-app-approval` Gecko asks the app first, so DRM is denied everywhere by default yet can be enabled in Settings; `android.cfg` no longer forces `media.eme.enabled=false`, which no switch could undo. Fenix compilation pending. |
