...
Current classification: **24 common / 37 android / 36 desktop-only / 0 straddlers = 97
...
| `patches/android/cookie-banner-controls.patch` | LW-M7-23 | Independent normal/private reject-only global controls and domain exceptions, with acknowledged storage changes and no implicit site-data clearing. Scoped composition with Sync and 13 actual-JS checks pass; a native private-session lifetime fence, target compilation, full Kotlin suite and device behavior remain pending. |
