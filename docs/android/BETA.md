...
| E7 | Signed with the release key, **v2 + v3, no v1**, fingerprint matches `SIGNING.md`; signed offline by a holder, never on the CI runner, and the key is no longer readable by that runner | [Signing handoff](evidence/lw-m6-01/RELEASE-HANDOFF.md), signed APK verification, custody confirmation | **Cryptographic and current CI-isolation checks met; historical offline custody not met.** All four signed APKs in `~/redoubt-signed/` pass published fingerprint, v2+v3/no-v1 and exact candidate payload checks. The user confirms signing on Fedora before migration. The [QEMU runner](CI-VM.md) now has no host home or key path, and the old host runner is retired. [Intake evidence](evidence/lw-m6-08/returned-signing-intake.json). Migration does not establish earlier offline signing; no exception has been approved. |
