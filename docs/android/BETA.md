...
| E7 | Signed with the release key, **v2 + v3, no v1**, fingerprint matches `SIGNING.md`; signed offline by a holder, never on the CI runner, and the key is no longer readable by that runner | [Signing handoff](evidence/lw-m6-01/RELEASE-HANDOFF.md), signed APK verification, custody confirmation | **Cryptographic checks met; custody not met.** All four signed APKs are now in `~/redoubt-signed/`; published fingerprint, v2+v3/no-v1 and exact candidate payload verification pass. The user confirms signing happened on the Fedora build host. Its release key still exists under the runner’s owner. [Intake evidence](evidence/lw-m6-08/returned-signing-intake.json). The off-host requirement remains in force; no exception has been approved. |
