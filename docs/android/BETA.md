...
| E3 | Smoke suite green on the emulator against the beta build, **except the two checks E12's decision deliberately leaves red** | `docs/android/evidence/lw-m7-06/` | **not met, one check outstanding.** Green on the shipping artifact: `--check-search`, `--check-aboutconfig`, `--check-strings` (both halves), `--check-no-gms`, `--check-no-adjust`. Red *by design*: `--first-run-capture` and `--check-no-remote-settings`, which assert zero Remote Settings traffic that E12 decided to keep — do not weaken them. Still genuinely open: `--check-no-suggest`, whose capture-side positive control cannot see a search that the app does run. `--check-update-privacy` passes its store-build half; its opt-in half is unproven. |
...
| E5 | Fenix unit tests subtract cleanly: `board.py --check-fenix-tests` exit 0 on results from this tree | `docs/android/evidence/lw-m7-06/check-fenix-tests.out` | **met 2026-09-06** — 598 classes / 5,426 tests, 93 failing = 90 environmental + 3 known-real + 0 unexpected. First time the suite has ever been run here. Re-run it for the actual beta build. |
...
**Where this stood at the end of 2026-09-06:** seven of twelve met — E1, E2, E4, E5,
E6, E11 and E12. Outstanding: **E3** (one real check, `--check-no-suggest`), and **E7 to
E10**, which need a person: release signing with v2+v3, a second key holder or a dated
