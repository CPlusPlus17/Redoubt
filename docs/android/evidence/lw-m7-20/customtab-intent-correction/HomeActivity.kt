...
import org.mozilla.fenix.components.LibreWolfUboPreinstaller
...
open class HomeActivity : LocaleAwareAppCompatActivity(), NavHostActivity, CrashActionDispatcher {
    ...
    final override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val blockedAccountActivity = this is AuthCustomTabActivity && !AccountServices.isEnabled
        if (blockedAccountActivity) {
            // Drop callback URL, custom-tab session id and saved auth UI before intent processing.
            intent = Intent(this, AuthCustomTabActivity::class.java)
        }
        val accountSafeSavedInstanceState = if (blockedAccountActivity) null else savedInstanceState

        // LibreWolf: the nimbus-cli tooling hook is removed, not left as "a NOOP
        // when launching normally". It is NOT inert on a release build:
        // ArgumentProcessor.createCommandLineArgs(Uri) accepts any non-http(s)
        // intent URI carrying `--nimbus-cli=true`, and will then
        // setExperimentsLocally() + applyPendingExperiments() from a
        // `--experiments=` JSON blob and persist the enrolments to the Nimbus
        // database. That is a local configuration channel that outlives the
        // launch and overrides the compiled-in FML defaults on the next start --
        // including fission.isolationStrategy, which Core.kt reads from the FML.
        // With the remote channel gone this was the last way to make the FML
        // defaults not be the effective values, so it goes too. Nothing else
        // calls initializeTooling(); FenixApplication.onCreate has already built
        // the Nimbus singleton via initializeNimbus() before any Activity runs,
        // so removing this line does not change when Nimbus is constructed.
        ...
        components.strictMode.allowViolation(StrictMode::allowThreadDiskReads) {
            ...
            super.onCreate(accountSafeSavedInstanceState)
        }
        ...
        breadcrumb(
            message = "onCreate()",
            data = mapOf(
                "recreated" to (accountSafeSavedInstanceState != null).toString(),
        ...
        SplashScreenManager(
            ...
            onSplashScreenFinished = { result ->
                ...
                if (accountSafeSavedInstanceState == null && shouldShowOnboarding) {
        ...
        if (!shouldShowOnboarding) {
            ...
            if (accountSafeSavedInstanceState == null) {
                navigateToHome(navHost.navController)
            }

            if (shouldNavigateToBrowserOnColdStart(accountSafeSavedInstanceState)) {
        ...
        if (intent?.getBooleanExtra(AccountServicesPreference.OPEN_SETTINGS, false) == true) {
        ...
        if (blockedAccountActivity) {
            AccountServicesPreference.openSettings(this)
            finish()
        }
    ...
    override fun onResume() {
        ...
        // LibreWolf (LW-M6-06): the opt-in update check. Foreground only (this is
        // onResume, there is no worker or timer), off unless the user turned it
        // on, at most once a day, and compiled out entirely on builds without a
        // verification key. Everything it does is in org.mozilla.fenix.lw.UpdateCheck.
        org.mozilla.fenix.lw.UpdateCheck.maybeRun(this, lifecycleScope)
    ...
    internal fun handleNewIntent(intent: Intent) {
        ...
        if (intent.getBooleanExtra(AccountServicesPreference.OPEN_SETTINGS, false)) {
            intent.removeExtra(AccountServicesPreference.OPEN_SETTINGS)
            navHost.navController.navigate(NavGraphDirections.actionGlobalSettingsFragment())
            return
        }
    ...
    private fun observeUboStartup() {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                var failureDialog: AlertDialog? = null
                try {
                    components.uboPreinstaller.state.collect { state ->
                        failureDialog?.dismiss()
                        failureDialog = null
                        if (state is LibreWolfUboPreinstaller.State.Failed) {
                            failureDialog = AlertDialog.Builder(this@HomeActivity)
                                .setTitle(R.string.librewolf_ubo_setup_failed_title)
                                .setMessage(R.string.librewolf_ubo_setup_failed_message)
                                .setCancelable(false)
                                .setPositiveButton(R.string.librewolf_ubo_setup_retry) { _, _ ->
                                    components.uboPreinstaller.retry()
                                }
                                .setNegativeButton(R.string.librewolf_ubo_setup_close) { _, _ ->
                                    finishAndRemoveTask()
                                }
                                .show()
                        }
