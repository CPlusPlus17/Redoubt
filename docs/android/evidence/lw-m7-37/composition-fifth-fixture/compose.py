#!/usr/bin/env python3
"""Replay Task37 on the frozen fifth-fixture plan; never stage or build it."""
import argparse
import collections
import gzip
import hashlib
...
def compose(repo):
    ...
    require(sha(parent['expected-current165-source-sha256.txt']) ==
            pins['expected_current165_manifest_sha256'], 'parent current manifest differs')
    final = manifest(parent['proposed-source-sha256.txt'])
    current = manifest(parent['expected-current165-source-sha256.txt'])
    ...
    stage.update(
        scope='HISTORICAL SOURCE PLAN ONLY, NOT STAGED. Newer fixture/product changes need a separate reviewed revision. Preflight every existing and absent path before any mutation.',
        parent_final230_manifest_sha256=sha(parent['proposed-source-sha256.txt']),
    ...
    outputs = {
        ...
        'expected-current165-source-sha256.txt': parent['expected-current165-source-sha256.txt'],
        'parent-final230-source-sha256.txt': parent['proposed-source-sha256.txt'],
        ...
        'live-preflight.json': json_bytes({
            ...
            'expected_current165_manifest_sha256': pins['expected_current165_manifest_sha256'],
    ...
    receipt = {
        ...
        'actor': '/root/entry_audit', 'script_sha256': sha(Path(__file__).read_bytes()),
        'inputs_sha256': sha((HERE / 'inputs.json').read_bytes()),
        'parent_handoff_sha256': sha(parent_archive),
        'parent_final230_manifest_sha256': pins['parent_final_manifest_sha256'],
        ...
        'additional_to_current165_preflights': len(before) - len(current),
        'retained_native_rule': 'All 146 unchanged native bodies keep their original hashes; bytes require live verification. This 97-file archive is not a complete Gecko tree.',
        'limits': 'Only frame and cookie primitives implemented. Full writer/cache admission, journal success and all target compilation/runtime remain pending. Future Task20/product/test corrections are excluded from this historical revision.',
