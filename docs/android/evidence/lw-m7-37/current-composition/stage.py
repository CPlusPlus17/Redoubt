...
CHECKPOINT = v.REPO / 'docs/android/evidence/lw-m7-12/current167'
sys.path.insert(0, str(CHECKPOINT))
import common as c
import runtime as prior_runtime

OUT = c.WORK / 'evidence/current245-source-stage'
SERVICE = 'redoubt-current245-source-stage-20260909.service'
...
def own_inputs():
    ...
    paths += [CHECKPOINT / n for n in ('common.py', 'build.py', 'runtime.py')]
    paths += [v.HERE.parent / 'composition-fifth-fixture/handoff/composed-source-subset.tar.gz']
    return [c.record(p) for p in paths]
...
def prepare():
    ...
    return {'schema': 1, 'kind': 'current245-source-stage', 'service_name': SERVICE,
...
def checked_config(path, digest):
    ...
    c.require(config['schema'] == 1 and config['kind'] == 'current245-source-stage'
...
def execute(path, digest):
    ...
    state = {'schema': 1, 'status': 'PREPARING', 'kind': 'current245-source-stage', 'service': observed,
