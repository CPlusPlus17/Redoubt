#!/usr/bin/env python3
"""Replay Task37 on isolated-process-corrected167 / proposed232; never stage or build it."""
import argparse
import collections
import gzip
import hashlib
...
def compose(repo):
    ...
    stage.update(
        scope='SOURCE PLAN ONLY, NOT STAGED. Exact Task37 increment on corrected current167 / proposed232; preserve historical243 and245 separately. Preflight every existing and absent path before any mutation.',
    ...
    receipt = {
        ...
        'limits': 'Only frame and cookie primitives implemented. Full writer/cache admission, journal success and all target compilation/runtime remain pending. Current167 Home routing, test and isolated-process corrections are included via the exact parent handoff; older native/compiler/test receipts remain historical.',
