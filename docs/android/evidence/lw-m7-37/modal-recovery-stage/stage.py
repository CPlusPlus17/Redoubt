...
v = module('modal245_composition', COMPOSITION / 'verify.py')
CHECKPOINT = v.REPO / 'docs/android/evidence/lw-m7-12/modal-process-recovery'
missing_common = object()
saved_common = sys.modules.get('common', missing_common)
try:
    c = module('modal245_checkpoint_common', CHECKPOINT / 'common.py')
    sys.modules['common'] = c
    prior_runtime = module('modal245_checkpoint_runtime', CHECKPOINT / 'runtime.py')
finally:
    if saved_common is missing_common:
...
def version_inputs():
    ...
    for key in ('parent_stage', 'parent_version', 'composition_inputs', 'composition_verifier'):
        row = pins[key]
    ...
    for name, row in pins['parent_helper_files'].items():
        observed = c.record(v.REPO / name)
        c.require(observed['sha256'] == row['sha256'] and observed['bytes'] == row['bytes'],
                  'retained parent helper input changed: ' + name)
...
def own_inputs():
    ...
    paths += [v.REPO / name for name in version['parent_helper_files']]
