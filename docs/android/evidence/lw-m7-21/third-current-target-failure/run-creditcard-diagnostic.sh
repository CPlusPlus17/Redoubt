#!/usr/bin/env bash
# Isolate the unchanged credit-card class after the archived full-suite failure.
set -euo pipefail
[[ $(systemd-detect-virt --vm) == kvm && $(id -un) == runner ]]
work=/home/runner/work/feature-parity-20260908
export XDG_RUNTIME_DIR=/run/user/1001
[[ -z $(podman --remote=false ps -q) ]]
evidence="$work/evidence/creditcard-diagnostic-20260909"
mkdir "$evidence"
...
[[ -d "$results" && ! -e "$results-third-current-20260909" ]]
mv "$results" "$results-third-current-20260909"
date -u --iso-8601=seconds > "$evidence/started.txt"
set +e
podman --remote=false run --rm --name creditcard-diagnostic-20260909 \
