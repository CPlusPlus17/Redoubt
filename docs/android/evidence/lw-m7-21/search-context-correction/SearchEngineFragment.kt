...
class SearchEngineFragment : PreferenceFragmentCompat(), SystemInsetsPaddedFragment {
    ...
    override fun onResume() {
        ...
        val suggestMaster = requirePreference<SwitchPreferenceCompat>(R.string.pref_key_enable_fxsuggest).apply {
            isChecked = context.components.settings.enableFxSuggest
        }
        val showSponsoredSuggestionsPreference =
            requirePreference<SwitchPreferenceCompat>(R.string.pref_key_show_sponsored_suggestions).apply {
                isChecked = context.components.settings.showSponsoredSuggestions
                summary = getString(R.string.firefox_suggest_local_data_required)
            }
        ...
        val cardSelectors = listOf(
            R.string.pref_key_search_optimization_stocks to requireContext().components.settings.shouldShowSearchOptimizationStockCard,
            R.string.pref_key_search_optimization_sports to requireContext().components.settings.shouldShowSearchOptimizationSportCard,
            R.string.pref_key_search_optimization_flights to requireContext().components.settings.shouldShowSearchOptimizationFlightCard,
        ).map { (key, selected) ->
            requirePreference<CheckBoxPreference>(key).apply {
                isPersistent = false
                isChecked = selected
                onPreferenceChangeListener = FirefoxSuggestPolicy.updater()
            }
        }
        ...
        fun updateSuggestControls() {
            val settings = requireContext().components.settings
            showSponsoredSuggestionsPreference.isEnabled = settings.enableFxSuggest
            showNonSponsoredSuggestionsPreference.isEnabled = settings.enableFxSuggest
            showSuggestionCardsPreference.isEnabled = settings.enableFxSuggest && settings.showNonSponsoredSuggestions
            cardSelectors.forEach {
                it.isEnabled = showSuggestionCardsPreference.isEnabled && settings.shouldShowSearchOptimizationCards &&
                    settings.isSearchOptimizationEnabled
            }
        }
        suggestMaster.onPreferenceChangeListener = FirefoxSuggestPolicy.updater(::updateSuggestControls)
        showSponsoredSuggestionsPreference.onPreferenceChangeListener = FirefoxSuggestPolicy.updater()
        showNonSponsoredSuggestionsPreference.onPreferenceChangeListener = FirefoxSuggestPolicy.updater(::updateSuggestControls)
        showSuggestionCardsPreference.onPreferenceChangeListener = FirefoxSuggestPolicy.updater(::updateSuggestControls)
        listOf(suggestMaster, showSponsoredSuggestionsPreference, showNonSponsoredSuggestionsPreference, showSuggestionCardsPreference)
            .forEach { it.isPersistent = false }
        updateSuggestControls()
    }
