"""Stage four reviewed test-only corrections while retaining compiled APK identity."""
import argparse
import datetime
import hashlib
import json
...
OUT = WORK / 'evidence/test-fixture-source'
PARENT = 'c53736e87152ba6de7ae232efd0a26197601027e4553be3f5f4bd7e6045601ae'
FILES = {
...
def main():
    ...
    assert digest(APK / 'source-sha256.txt') == PARENT == parent['source_manifest_sha256']
    assert digest(APK / 'native-input-sha256.txt') == parent['files']['parity-extended-apk/native-input-sha256.txt']['sha256']
    before = parse_manifest(APK / 'source-sha256.txt')
    ...
    inputs_path = HERE / 'test-fixture-corrections/inputs.json'
    inputs = json.loads(inputs_path.read_text())
    assert inputs['parent_manifest_sha256'] == PARENT
    ...
    assert len(inputs['files']) == 4 and set(fixes) == FILES
    after = dict(before)
    ...
    binding = {'parent_manifest_sha256': PARENT, 'inputs_sha256': digest(inputs_path),
    ...
    if OUT.exists():
        ...
        print('PASS existing four test fixture corrections; compiled APKs unchanged')
        return
    ...
    assert (WORK / 'evidence/apk-resource-recovery-checkpoint/tests-exit.txt').read_text().strip() == '1'
    ...
    result = {'status': 'PASS', 'binding': binding, 'source_count': 165, 'test_only_changes': inputs['files'],
              'source_manifest_sha256': digest(OUT / 'source-sha256.txt'),
              'finished': datetime.datetime.now(datetime.timezone.utc).isoformat(),
              'scope': 'Four test source files changed; all production source and compiled APK bytes unchanged. Full target rerun and runtime remain pending.'}
    (OUT / 'receipt.json').write_text(json.dumps(result, indent=2) + '\n')
    print('PASS staged four test fixture corrections; 165 source bindings and compiled APK identities verified')
