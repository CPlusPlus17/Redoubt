...
mkdir -p "$evidence"
cd "$work/src"
sha256sum -c "$work/evidence/extended-source/source-sha256.txt" > "$evidence/source-before.txt"
cd "$work/repo"
...
set -e
cd "$work/src"
sha256sum -c "$work/evidence/extended-source/source-sha256.txt" > "$evidence/source-after.txt"
