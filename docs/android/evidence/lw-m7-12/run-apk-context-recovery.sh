#!/usr/bin/env bash
# Retry the Fenix context correction; preserve native4 and failed APK evidence.
set -euo pipefail
...
python3 "$work/repo/docs/android/evidence/lw-m7-21/stage-search-context.py"
resource="$work/evidence/apk-search-context-source"
if [[ -d "$evidence" ]]; then
  mv "$evidence" "$evidence-previous-$(date -u +%Y%m%dT%H%M%S)-$$"
fi
mkdir -p "$evidence"
cp "$resource/source-sha256.txt" "$evidence/source-sha256.txt"
cp "$resource/receipt.json" "$evidence/search-context-staging.json"
...
sha256sum scripts/android-apk.sh assets/mozconfig.android \
  docs/android/evidence/lw-m7-21/stage-search-context.py \
