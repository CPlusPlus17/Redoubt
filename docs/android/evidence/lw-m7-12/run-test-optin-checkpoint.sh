...
cd "$work/repo"
evidence="$work/evidence/test-optin-recovery-checkpoint"
mkdir "$evidence"
...
for stage in tests runtime; do
  case "$stage" in
    tests) script=docs/android/evidence/lw-m7-12/run-optin-tests.sh ;;
    runtime) script=docs/android/evidence/lw-m7-12/run-optin-smoke.sh ;;
  esac
  set +e
