...
cd "$work/src"
sha256sum -c "$apk_evidence/source-sha256.txt" > "$evidence/source-input-check.txt"
...
sha256sum "$apk" scripts/android-smoke.sh scripts/android-graphics-smoke.py \
  scripts/android-pref-audit.sh docs/android/evidence/lw-m7-12/run-candidate-smoke.sh \
