#!/usr/bin/env bash
# Retry the permission Bundle correction; preserve native4 and failed APK evidence.
set -euo pipefail
...
python3 "$work/repo/docs/android/evidence/lw-m7-21/stage-permission-bundle.py"
resource="$work/evidence/apk-permission-bundle-source"
if [[ -d "$evidence" ]]; then
  mv "$evidence" "$evidence-previous-$(date -u +%Y%m%dT%H%M%S)-$$"
fi
mkdir -p "$evidence"
cp "$resource/source-sha256.txt" "$evidence/source-sha256.txt"
cp "$resource/receipt.json" "$evidence/permission-bundle-staging.json"
cp "$work/evidence/apk-search-context-source/receipt.json" "$evidence/search-context-staging.json"
cp "$work/evidence/apk-admission-copy-source/receipt.json" "$evidence/admission-copy-staging.json"
