...
def main():
    ...
    for source in manifest["inspected_sources"]:
        require(digest((HERE / source["retained_excerpt"]).read_bytes()) == source["excerpt_sha256"],
                "inspected source excerpt changed: " + source["source_path"])
    ...
    with tempfile.TemporaryDirectory(prefix="lw-m7-26-source-") as directory:
