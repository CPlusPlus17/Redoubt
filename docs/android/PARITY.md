...
| 6 | DNS-over-HTTPS | ON | **Equivalent** | Code-locked in `GeckoRuntimeSettings.java` — the only network pref in the `[ANDROID: LOCK]` set, hence honoured regardless of cfg packaging. Measured by: source + config parse. [network-posture] |
...
| 10 | Global Privacy Control | ON | **Equivalent** | Code-locked (`[ANDROID: LOCK]` set, `ContentBlocking.java`) — honoured regardless of cfg packaging. Measured by: source. [RFP-ANDROID] |
| 11 | Enhanced Tracking Protection (strict) | ON | **Equivalent** | Code-locked (`[ANDROID: LOCK]` set) — honoured regardless of cfg packaging. Measured by: source + config. [RFP-ANDROID, POLICIES] |
| 12 | Cookie partitioning / SameSite | ON | **Equivalent** | Part of the `[ANDROID: LOCK]` code-locked set (with DoH, GPC, ETP) — honoured regardless of cfg packaging. Measured by: source. [network-posture] |
