...
| 5 | HTTPS-only mode | ON | **Partial** | `upgrade_insecure_requests` + `upgrade_local` + error-page-suggestions configured; the main switch is not force-set (a desktop design choice, not an Android regression); the cfg layer is packaged and applied on Android (LW-M3-08/LW-M3-10, runtime canary locked since `lw-fresh-2026-08-22`), but unlocked defaults can still be rewritten by Fenix at startup (AGENTS.md L2) — only `[ANDROID: LOCK]` prefs are certain; `privacy-defaults.patch` (LW-M7-09) sets Fenix's own HTTPS-only default, live behaviour PENDING (FEATURE-PARITY F02). Measured by: config parse (network-check.sh). [network-posture §1–2] |
| 6 | DNS-over-HTTPS | OFF by choice (mode 5), Quad9 preset, dns4all fallback | **Equivalent in source (Beta 2); live PENDING** | **Updated 2026-09-24:** `privacy-defaults.patch` (LW-M7-09, in Beta 2) makes Fenix's own defaults LibreWolf's — mode 5 and LibreWolf's provider list with Quad9 and the DNS4All fallback — so the startup push below now carries our values; stored user choices are kept. Resolver behaviour on a device is still unmeasured (FEATURE-PARITY F05). The original Beta 1 finding follows. `common.cfg:259-260` set `network.trr.mode=5` and the Quad9 URI, but `Core.kt:190-191` → `GeckoEngine.kt:2032-2033` overwrite both on every cold start with Fenix's own defaults (mode 0, empty URI); the equality guard cannot save them because `PrefWithoutDefault` starts at null. Locking is not the remedy: Settings > Privacy > DNS over HTTPS owns both prefs, so a lock would leave the provider list interactive while discarding every choice (`must-not-lock.txt`, `network.trr.*`). The user can turn DoH on in that screen; our configured provider does not reach Gecko. Fix is Fenix-side and owned by no task yet. Measured by: code trace, commit 7c92b22. [must-not-lock.txt] |
| 7 | Certificate revocation (CRLite + OCSP off) | ON (CRLite mode 2, OCSP disabled) | **Partial** | Cfg-only; the cfg is applied on Android (LW-M3-08 resolved), live behaviour PENDING. Naming note: LibreWolf ships **CRLite mode 2 with OCSP disabled**, not "OCSP hard-fail" (report item e). Measured by: config parse. [network-posture §5] |
| 8 | Modern TLS floor (min ≥ 3, no 0-RTT, no deprecated ciphers) | ON | **Partial** | Cfg-only; the cfg is applied on Android (LW-M3-08 resolved), live behaviour PENDING. Measured by: config parse (network-check.sh). [network-posture] |
...
row 1's absent content-process sandbox and row 6's DoH (overwritten by Fenix in
Beta 1, fixed in source for Beta 2, live behaviour still unmeasured). Publishing the sentence without a reachable link to that table
would make it false.
