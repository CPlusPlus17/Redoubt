...
**And the sting in the tail, found by LW-M3-01: `MOZ_DEFAULT_PREFS` cannot express
a lock at all.** `GeckoLoader.setupInitialPrefs` builds `pref("name", value);`
lines, `Preferences.cpp:3963` parses them with `PrefValueKind::Default`, and
`modules/libpref/parser/src/lib.rs:14,297-321` accepts only `pref`, `user_pref` and
`sticky_pref` — **there is no `locked_pref` token**. So the channel this landmine
tells you to rely on has no way to do the one thing the landmine says is mandatory.
All 38 `lockPref`s in `common.cfg` need a real `Preferences::Lock` call from
LW-M3-03, or they are advisory only.
